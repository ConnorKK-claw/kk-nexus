...
def cmd_health(args):
    ...
    if args.vault:
        ...
        # No vault specified: run auto_health (scans all)
        return run_script("auto_health.py", [], dry_run=args.dry_run)
...
def cmd_fetch_prices(args):
    """Fetch historical stock prices from eastmoney."""
    return run_script("fetch_market_data.py", [], dry_run=args.dry_run)
def cmd_gen_comparison(args):
    """Generate case comparison report."""
    return run_script("gen_bb0_comparison.py", [], dry_run=args.dry_run)
...
def cmd_search(args):
    ...
    return 0
def cmd_calc_ratio(args):
    """Calculate executive grant ratio."""
    return run_script("calc_exec_ratio.py", [], dry_run=args.dry_run)
def cmd_fix_stock_source(args):
    """Fix stock source data consistency."""
    return run_script("fix_stock_source.py", [], dry_run=args.dry_run)
def cmd_auto_distill(args):
    """Run auto-distillation check for wiki content."""
    return run_script("auto_distill.py", [], dry_run=args.dry_run)
...
def cmd_learning_bridge(args):
    """Bridge .learnings/ entries to vault journals."""
    extra = []
    if args.dry_run: extra.append("--dry-run")
    if args.suggest: extra.append("--suggest")
    return run_script("bridge_learnings.py", extra, dry_run=False)
# ── Argument parser ──────────────────────────────────────────────
...
Examples:
  ...
  python kam.py search '股权激励'
    ...
    p_fp = sub.add_parser("fetch-prices", help="Fetch historical stock prices")
    p_fp.add_argument("--dry-run", "-n", action="store_true")
    p_gc = sub.add_parser("gen-comparison", help="Generate case comparison report")
    ...
    p_cr = sub.add_parser("calc-ratio", help="Calculate executive grant ratio")
    p_cr.add_argument("--dry-run", "-n", action="store_true")
    p_fs = sub.add_parser("fix-stock-source", help="Fix stock source data")
    p_fs.add_argument("--dry-run", "-n", action="store_true")
    p_ad = sub.add_parser("auto-distill", help="Run auto-distillation check")
    ...
    p_lb = sub.add_parser("learning-bridge", help="Bridge .learnings/ to vault journals")
    p_lb.add_argument("--dry-run", "-n", action="store_true", help="Preview only")
    p_lb.add_argument("--suggest", action="store_true", help="Suggest knowledge node candidates")
...
def main():
    ...
    handlers = {
        ...
        "gen-comparison": cmd_gen_comparison,
        "update-cases": cmd_update_cases,
        "search": cmd_search,
        "calc-ratio": cmd_calc_ratio,
        "fix-stock-source": cmd_fix_stock_source,
...
def cmd_learning_bridge(args):
    ...
    if args.dry_run: extra.append("--dry-run")
    if args.suggest: extra.append("--suggest")
    return run_script("bridge_learnings.py", extra, dry_run=args.dry_run)