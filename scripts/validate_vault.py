...
VALID_SOURCES = {"user", "agent", "distilled", "ima", "ima-knowledge-base"}
VALID_STATUSES = {"raw", "distilled", "verified", "expired", "archived"}
REQUIRED_FIELDS = ["title", "date", "source", "domain", "tags", "status"]

# 不参与知识节点 schema 校验的路径/文件
SKIP_PATH_PREFIXES = (".learnings/", "_wiki_ingest/", "memory/", "journal/")
SKIP_ROOT_FILES = {
    "SESSION-STATE.md",
    "working-buffer.md",
    "MEMORY.md",
...
def validate_vault(vault_dir: Path, strict: bool = False, fix: bool = False, warn: bool = False) -> tuple[int, int, int]:
    ...
    for f in sorted(md_files):
        ...
        # 跳过项目元数据与第三方导入目录
        if rel_path.startswith(SKIP_PATH_PREFIXES):
            continue
        if f.parent == vault_dir and f.name in SKIP_ROOT_FILES:
            continue
        ...
        # 模板文件不强制使用知识节点 schema，仅检查命名规范
        if parent_dir == "templates":
            if not PATTERNS["templates"].match(f.name):
                file_warnings.append("文件名不符合 templates/ 命名规范")
            if file_warnings:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--warn", action="store_true", help="仅警告：即使有错误也返回 0，不阻断后链")
    args = parser.parse_args()

    passed, warnings, errors = validate_vault(args.vault_path, strict=args.strict, fix=args.fix, warn=args.warn)
    if errors > 0 and not args.warn:
        sys.exit(1)
