...
# Get 上证指数 data via yfinance
print("Fetching 上证指数 from yfinance...")
index_ticker = "000001.SS"  # Yahoo Finance code for 上证指数

# All the date ranges we need
idx = yf.download(index_ticker, start="2017-10-01", end="2026-05-20", progress=False, auto_adjust=True)
...
if idx is not None and len(idx) > 0:
    for date, row in idx.iterrows():
        ...
        idx_data[d] = close
    print(f"Got {len(idx_data)} index days")
    ...
    for i, d in enumerate(sorted_dates):
        if i == 0:
            ...
            prev_close = idx_data[sorted_dates[i-1]]
            pct = round((idx_data[d] - prev_close) / prev_close * 100, 2)
