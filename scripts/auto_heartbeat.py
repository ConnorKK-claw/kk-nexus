...
Performs:
1. Run auto_health.py (import as module or subprocess)
2. Check .learnings/ ERRORS.md for new entries (within 7 days)
3. Run bridge_learnings.py to sync .learnings/ to vault journal
...
PROJECT_ROOT = Path.home() / "OneDrive" / "文档" / "New project 6"
LOGS = PROJECT_ROOT / "logs"
LOGS.mkdir(exist_ok=True)
VAULT_PATH = Path.home() / ".codex" / "skills" / "equity-incentive" / "vault"

def run_health() -> str:
    """Run auto_health.py and capture its output."""
    health_script = PROJECT_ROOT / "scripts" / "auto_health.py"
    if not health_script.exists():
        return "[ERR] auto_health.py not found"
    try:
        r = subprocess.run(
        ...
        return "[ERR] auto_health.py timed out (>120s)"
    except Exception as e:
        return f"[ERR] auto_health.py error: {e}"


def run_bridge_learnings() -> str:
    """Run bridge_learnings.py to sync .learnings/ to vault journals."""
    script = PROJECT_ROOT / "scripts" / "bridge_learnings.py"
    if not script.exists():
        return "[ERR] bridge_learnings.py not found"
    try:
        r = subprocess.run(
        ...
        return f"[ERR] bridge_learnings error: {e}"
