...
def build_authors_index(vault_path: Path, dry_run: bool = False):
    ...
    raw_agent_dir = vault_path / "raw" / "agent"
    ...
    def scan_raw_dir(dir_path: Path, rel_prefix: str):
        nonlocal total_raw
        ...
        for f in sorted(dir_path.glob("*.md")):
            ...
            if not authors and institution:
                authors = [institution]
            for author in authors:
                author_articles[author].append((rel_path, title, date, tags, source))
            if institution:
                inst_articles[institution].append((rel_path, title, date, authors[0] if authors else ""))
    
    scan_raw_dir(raw_dir, "raw/user")
    scan_raw_dir(raw_agent_dir, "raw/agent")
    ...
    output_parts = []
    output_parts.append(f"# {skill_name} 作者索引")
    output_parts.append("")
